  ...
  for (i in 1:N) {
    for (t in 1:Tsubj[i]) {
      ...
      if (cert[i,t] < 0){ // If loss trials only (sure option is negative)
        evSafe   = - (1.0 * pow(abs(cert[i, t]), rho[i])); // applies risk and loss aversion to negative sure option and negate
        evGamble = - 0.5 * 1.0 * pow(abs(loss[i, t]), rho[i]); //Gain is always Zero
      }
      if (cert[i,t] == 0){ // mixed gamble trials (sure option is exactly 0)
        evSafe   = pow(cert[i, t], rho[i]);
        evGamble = 0.5 * (pow(gain[i, t], rho[i]) - 1.0 * pow(abs(loss[i, t]), rho[i]));
      }
