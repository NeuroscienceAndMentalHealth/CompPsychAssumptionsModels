  ...
  for (i in 1:N) {
    for (t in 1:Tsubj[i]) {
      ...
      if (cert[i,t] < 0){ // If loss trials only (sure option is negative)
        evSafe   = - (lambda[i] * pow(fabs(cert[i, t]), rho[i])); // applies risk and loss aversion to negative sure option and negate
        evGamble = - 0.5 * lambda[i] * pow(fabs(loss[i, t]), rho[i]); //Gain is always Zero
      }
