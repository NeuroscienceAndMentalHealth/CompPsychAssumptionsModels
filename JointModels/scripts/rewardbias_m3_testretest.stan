...
transformed parameters {
      ...
      matrix<lower=0> [N,N_time] i_sens;
      ...
      for (time in 1:N_time){
        for (i in 1:N) {
          ...
          i_sens[i, time] = Phi_approx(mu_i_sens[time] + i_sens_tilde[time,i]) * 5;
...
model {
      ...
      for (time in 1:N_time){
        for (i in 1:N) {
          ...
          v = [rep_row_vector(initV[i,time],levels),rep_row_vector(1-initV[i,time],levels)];
          
          for (t in 1:T) {
            vector [2] tempv;
            tempv = [v[1,congruence[i,time,t]],v[2,congruence[i,time,t]]]';
           	choice[i, time, t] ~ categorical_logit(tempv + i_sens[i, time] * correct[i, time, t]);
            v[choice[i, time, t],congruence[i,time,t]] = v[choice[i, time, t],congruence[i,time,t]]+ alpha[i, time] * (r_sens[i, time] * rwd[i, time, t]-v[choice[i, time, t],congruence[i,time,t]]);
          }
      ...
      for (time in 1:N_time){
        for (i in 1:N) {
          ...
          for (t in 1:T) {
            vector [2] tempv;
            tempv = [v[1,congruence[i,time,t]],v[2,congruence[i,time,t]]]';
           	log_lik[i,time] = log_lik[i,time] + categorical_logit_lpmf( choice[i,time,t] | (tempv + i_sens[i, time] * correct[i, time, t]));
            v[choice[i, time, t],congruence[i,time,t]] = v[choice[i, time, t],congruence[i,time,t]]+ alpha[i, time] * (r_sens[i, time] * rwd[i, time, t]-v[choice[i, time, t],congruence[i,time,t]]);
          }
